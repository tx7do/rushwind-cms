...
/// The refresh-token cookie's narrowed path (the admin BFF's refresh
/// endpoint).
pub const REFRESH_COOKIE_PATH: &str = "/admin/v1/refresh-token";
...
mod tests {
    ...
    fn cookie_values_carry_the_reference_attributes() {
        ...
        assert!(rt.starts_with("refresh_token=tok; Path=/admin/v1/refresh-token; Max-Age=43200"));
        ...
        assert!(rt.starts_with("refresh_token=; Path=/admin/v1/refresh-token; Max-Age=0"));
