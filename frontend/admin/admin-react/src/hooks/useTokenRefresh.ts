...
export function reconnectSSEServer(): void {
  ...
  // CMS 后端契约：stream 必须与令牌一致（HandleAuthorize 校验 streamID != token 即 403），
  // 鉴权另走 Authorization 头。缺一不可。
  if (!accessToken) {
  ...
  const sseUrl = `${import.meta.env.VITE_SSE_URL ?? '/api/sse'}?stream=${encodeURIComponent(accessToken)}`;
