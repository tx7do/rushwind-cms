...
async function applySuccessfulLogin(
  ...
  // 建立 SSE 连接（对齐 vben 登录成功行为：vben authentication.store.ts 在登录成功后连 SSE）
  connectSSEServer();
...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      login: async (params, onSuccess, captcha) => {
        ...
        try {
          ...
          // mfa_operation_id 不在 CMS 生成的 LoginResponse 类型上（基座契约差异），
          // 后端若启用 MFA 会运行时携带；类型层用 any 桥接。
          const mfaOperationId = (response as { mfa_operation_id?: string })
            .mfa_operation_id;
          if (mfaOperationId) {
            set({ mfaOperationId });
      ...
      logout: async (_redirect = true) => {
        ...
        try {
          ...
          // 清除内存中的用户态（身份/角色/权限码）。
          // 只删 storage 键不清内存的话，同 tab 内换账号登录会经
          // useAuth.getUserPermissionCodes 的缓存跳过分支继承上一会话的
          // 角色与权限码（跨登录权限残留，实测平台→租户切换可复现）。
          useUserStore.getState().$reset();
          // 布局态标签页同源问题：残留 tab 跨会话存续且失效（D14），
          // 一并清内存与持久化键。
      ...
      refreshToken: async () => {
        try {
          // CMS 的 refresh token 为不透明随机串，绑定键 (uid/jti) 存在于 access
          // token payload 中。从持久化的 access token 解出后随请求回传
          // （uid/jti 非机密，出现在每个 bearer 里；真正的凭证是 HttpOnly cookie
          // 中的 refresh token 值）。
          let binding: { userId?: number; jti?: string } | undefined;
          ...
          if (at) {
            try {
              ...
              binding = { userId: payload.uid, jti: payload.jti };
            } catch {
              /* 解码失败则空绑定键，由服务端判定 */
            }
          }
          const response = await refreshTokenMutation.execute(binding);
      ...
      forceLogout: () => {
        ...
        localStorage.removeItem('auth-storage');
        localStorage.removeItem('user-storage');
        // 同 logout：内存态用户数据一并清（防跨登录权限残留）
        useUserStore.getState().$reset();
        // 同 logout：布局态标签页一并清（D14）
        useTabsStore.setState({ tabs: [] });
        localStorage.removeItem('app-tabs');
        set({
          accessToken: null,
    ...
    }),
    {
      ...
      // 持久化 access token 及过期时间：刷新/关闭标签后恢复登录态（对齐 vben 侧体验）。
      // 双重作用：① 深链刷新时 initRouter 发现已认证，按真实权限重建路由；
      // ② 其 payload 里的 uid/jti 是刷新请求的绑定键（CMS refresh token 为不透明
      // 随机串，cookie 只存凭证值）。会话过期由既有链路兜底：API 401 → 凭
      // HttpOnly refresh cookie 静默续期；cookie 失效则 forceLogout 清空本存储。
      partialize: (s) => ({
