// 读取 .env.development 后再启动 next dev。
// Next CLI 解析端口时 .env 文件尚未加载（PORT 只认进程环境变量），
// 因此在这里提前注入，使 dev server 端口可在 .env.development 中配置。
...
// 探测端口在指定地址上可否绑定。next dev 默认 hostname 0.0.0.0，但实际
